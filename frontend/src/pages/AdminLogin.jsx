...
const AdminLogin = () => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#FBF7F0]">
      ...
      {/* Soft warm overlay — improves text contrast on mobile where image crops differently */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#FBF7F0]/40 via-transparent to-[#FBF7F0]/60 lg:hidden" />

      {/* ─── Content layer ───────────────────────────────────────── */}
      <div className="relative z-10 min-h-screen flex flex-col lg:flex-row">

        {/* ─── LEFT: Brand + editorial text ────────────────────── */}
<div className="lg:w-[55%] flex flex-col px-6 py-8 sm:px-10 sm:py-10 lg:p-12 xl:p-16">

  {/* Brand — top-left */}
  <img
    src="/kritya-logo-2.png"
    alt="Kritya's Admin Portal"
    className="h-12 sm:h-14 lg:h-16 w-auto self-start"
  />

  {/* Editorial text — pinned to upper-middle area (calm zone) */}
  <div className="mt-10 sm:mt-14 lg:mt-20 xl:mt-24 relative">
    {/* Soft cream halo behind the text for guaranteed readability */}
    <div
      className="absolute -inset-x-6 -inset-y-6 -z-10 rounded-[40px] pointer-events-none"
      style={{
        background:
          'radial-gradient(ellipse at left center, rgba(251, 247, 240, 0.85) 0%, rgba(251, 247, 240, 0.55) 45%, rgba(251, 247, 240, 0) 75%)',
      }}
    />

    <div className="max-w-[440px]">
      <div className="h-px w-12 bg-[#B8862F]/60 mb-6 sm:mb-7" />
      <p className="text-[10px] tracking-[0.35em] uppercase text-[#6E6355] mb-4 sm:mb-5 font-medium">
        The Atelier, Backstage
      </p>
      <h2 className="font-playfair text-[32px] sm:text-[40px] lg:text-[44px] xl:text-[50px] leading-[1.08] text-[#1F1A12]">
        Manage the craft
        <br />
        with <span className="italic text-[#B8862F]">clarity</span>.
      </h2>
      <p className="mt-5 sm:mt-6 text-[13px] sm:text-[14px] leading-[1.65] text-[#4A4034] max-w-[360px]">
        Collections, orders, inventory, and customers — all in one place, beautifully organized.
      </p>
    </div>
  </div>

  {/* Spacer bottom — keeps the layout balanced on desktop */}
            ...
            <div className="bg-white rounded-2xl border border-[#F2EADB] shadow-[0_24px_70px_-24px_rgba(184,134,47,0.20)] px-7 sm:px-10 py-8 sm:py-10">
              ...
              <div className="mt-7 pt-5 border-t border-[#F2EADB] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[12px]">
                <span className="text-[#8F8371]">
                  © {new Date().getFullYear()} Kritiya's
                </span>
                <a
                  href="/"
